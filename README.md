...
> **No API key is required to run the full app.** Perenual and Trefle keys
> are optional enrichment sources — everything else (Open-Meteo, GBIF,
...
> (see [AI features](#ai-features-optional)).
...
- A `Procfile` is included (`gunicorn run:app --bind 0.0.0.0:$PORT
  --timeout 120`, leaving room for slow AI replies), so platform hosts can
  start the app out of the box
- **Render** (free web service tier), **PythonAnywhere** (free tier) or a
  **Hugging Face Space** (Docker) are the easiest starting points for a
  Flask app like this
- On first boot, tables auto-create and an empty database auto-seeds from
  `data/plants.json`, so hosts with ephemeral disks (Render's free tier
  wipes the SQLite file on every deploy) recover on their own
